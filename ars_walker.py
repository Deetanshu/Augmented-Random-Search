...
def train(env, policy, normalizer, hp):
    ...
    for step in range(hp.nb_steps):
        ...
        print('Step: ', step, 'Reward: ', reward_evaluation, 'Reward-previous: ', hp.prev_reward)
        if reward_evaluation > 1000 and hp.prev_reward > 1000:
            break
        hp.prev_reward=reward_evaluation
...
def mkdir(base, name):
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    return path
work_dir = mkdir('exp', 'ars-walkers2')
...
np.savetxt('perceptron2.txt', policy.theta, fmt = '%d')
